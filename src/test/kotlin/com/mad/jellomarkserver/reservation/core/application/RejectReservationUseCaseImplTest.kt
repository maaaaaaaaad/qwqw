...
class RejectReservationUseCaseImplTest {
    ...
    @Test
    fun `should reject reservation with reason`() {
        ...
        assertEquals(ReservationStatus.REJECTED, result.status)
        assertEquals("해당 시간에 예약이 불가합니다", result.rejectionReason?.value)
    }
